...
class BoardWidget extends Pad {
    ...
    /** Draw the bare board G. Fix me. */
    private void drawGrid(Graphics2D g) {
        g.setColor(LIGHT_SQUARE_COLOR);
        g.fillRect(0, 0, BOARD_SIDE, BOARD_SIDE);
        for (int k = 0; k <= 2 * BOARD_SIDE; k += 2) {
            g.drawLine(cx(k), cy(0), cx(k), cy(2 * BOARD_SIDE));
            g.setColor(DARK_SQUARE_COLOR);
        }
        for (int k = 0; k <= 2 * BOARD_SIDE; k += 2) {
            g.drawLine(cx(0), cy(k), cx(2 * BOARD_SIDE), cy(k));
        }
    }
    ...
    public synchronized void paintComponent(Graphics2D g) {
        drawGrid(g);
        drawQueen(g, sq(4 * 10), WHITE);
        drawQueen(g, sq(3), WHITE);
        drawQueen(g, sq(6), WHITE);
        drawQueen(g, sq(4 * 10 + 9), WHITE);
        drawQueen(g, sq(7 * 10), BLACK);
        drawQueen(g, sq(9 * 10 + 3), BLACK);
        drawQueen(g, sq(9 * 10 + 6), BLACK);
        drawQueen(g, sq(7 * 10 + 9), BLACK);
    }
    ...
    /** Handle a click on S. Fix me. */
    private void click(Square s) {
        repaint();
        _commands.offer(s.toString());
    }
